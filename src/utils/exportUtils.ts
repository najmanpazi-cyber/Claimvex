...
export function buildCsvContent(result: CodingResult, request?: CodingRequest | null): string {
  ...
  // Header metadata
  rows.push(csvRow(["ClaimVex Coding Export", ts]));
  ...
  rows.push(csvRow(["DISCLAIMER", "ClaimVex provides coding suggestions only. All codes must be verified by a qualified coder before claim submission.", "", "", "", "", ""]));
...
export function downloadCsv(result: CodingResult, request?: CodingRequest | null): void {
  ...
  const filename = `claimvex-${primaryCode}-${dateStr}.csv`;
...
<head>
  <meta charset="UTF-8" />
  <title>ClaimVex Coding Summary — ${fullCode}</title>
  <style>
...
<body>
  <div class="header">
    <div>
      <div class="logo">ClaimVex <span>Coding Summary</span></div>
  ...
  <div class="disclaimer">
    ClaimVex provides coding suggestions only. All codes must be reviewed and verified by a qualified, credentialed coder or billing specialist before claim submission.
    This report does not constitute a legal, compliance, or billing determination. Generated by ClaimVex — ${ts}
  </div>
