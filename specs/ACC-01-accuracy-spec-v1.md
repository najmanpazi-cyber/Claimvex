# ACC-01: Accuracy Specification v1
...
| 2 | **Modifier 59/X** | -59 used on Medicare claim instead of -XE/-XS/-XP/-XU | High | High | Universal across all groupings; Medicare auto-adjudication rejects -59 in many MACs |
...
- **Action type:** **Block** — set `clean_claim_ready: false`
...
- **Expected behavior:** Suppress the injection code or flag as bundled.
- **Action type:** **Warn** — add `warnings[]` entry with type `"error"`
- **User-facing message:** "Joint injection is bundled with arthroscopy of the same joint on the same day per NCCI. The injection cannot be billed separately unless performed on a different joint (use -XS)."
...
- **Expected behavior:** Suppress the cast/splint code.
...
- **Expected behavior:** Suppress the diagnostic code.
- **Action type:** **Block** — remove diagnostic code from output or add `"error"` warning
...
- **Expected behavior:** Compare units against MUE table. Flag when units exceed the MUE.
- **Action type:** **Block** — set `clean_claim_ready: false`
- **User-facing message:** "Units of service ({units}) for CPT {code} exceed the Medicare MUE limit of {mue_limit}. The claim will be denied for units above the threshold. Verify the unit count and consider whether separate lines with modifiers are appropriate."
...
- **Expected behavior:** Replace -59 with the most specific X-modifier (XE/XS/XP/XU) based on context.
- **Action type:** **Force-review** — apply the X-modifier, add `payer_note` explaining the substitution
- **User-facing message:** "Medicare requires NCCI subset modifiers (-XE, -XS, -XP, -XU) instead of -59. We've applied -{x_modifier} based on the documentation. Verify this is the correct distinction before submitting."
...
- **Action type:** **Block** — set `clean_claim_ready: false`, add to `missing_information[]`
...
- **Expected behavior:** Require -25 or suppress the E/M.
- **Action type:** **Force-review** — suggest -25, flag documentation requirement
...
- **Expected behavior:** Suggest -57.
- **Action type:** **Force-review** — suggest -57, explain requirement
- **User-facing message:** "E/M on the day of or day before major surgery (90-day global) requires modifier -57 to indicate the visit included the decision for surgery. Without -57, the E/M is included in the surgical global."
...
- **Expected behavior:** Flag missing information, reduce confidence to "medium" or "low."
...
- **Action type:** **Block** — replace diagnosis, add warning
...
- **Expected behavior:** Flag documentation gap.
- **Action type:** **Warn** — add to `missing_information[]`, reduce confidence
...
| MUE unit limits | CMS MUE values are the industry ceiling; no commercial payer allows more |
...
|------|--------------------|--------------------|----------------|
| **Modifier -59 vs X-modifiers** | -59 accepted; X-modifiers optional but preferred | -59 rejected by many MACs; XE/XS/XP/XU required | When payer = Medicare, auto-substitute -59 → most-specific X-modifier. When payer = Commercial, accept -59 with informational note. |
...
| **-57 timing** | Some commercial payers accept -57 only on day-of surgery | Medicare accepts -57 on day-of or day-before major surgery | When payer = Commercial, flag if -57 is day-before (potential denial). When payer = Medicare, accept day-before. |
| **Bilateral billing format** | Most accept -50 on single line | Some MACs prefer separate lines with -LT/-RT; others accept -50. MAC-variable. | When payer = Medicare, add `payer_note`: "Verify MAC preference for bilateral format (-50 vs separate lines with -LT/-RT)." |
...
| -59 | Accepted with operative note showing distinct service | Not accepted at many MACs; must use XE/XS/XP/XU. Even when accepted, requires documentation of *which* distinction applies. | Medicare -59 denials are auto-adjudicated; appeal requires resubmission with correct X-modifier. |
| -22 | Requires operative note documenting increased complexity | Requires operative note + comparison to typical case; some MACs require additional letter | Appeal success rate ~40% for Medicare; commercial varies by payer. |
| -50 | Single line with -50 generally accepted | MAC-variable: some require two lines with -LT/-RT at 100%+100%; others accept -50 at 150% | Format denial — resubmit in correct format. No clinical appeal needed. |

**Safe defaults when payer is unknown or "Not specified":**
- Use X-modifiers over -59 (strictest rule wins)
- Require laterality documentation (universal)
- Apply Medicare-level -25 documentation threshold (conservative)
- Flag bilateral format ambiguity for user to verify with payer
...
| 2 | Every rule has exactly one action type: `block`, `warn`, or `force-review` | Section 3 review — no rule has ambiguous or missing action type |
| 3 | Every `block` rule sets `clean_claim_ready: false` in expected behavior | Section 3 review |
...
| 7 | Safe defaults are defined for when payer is unknown | Section 4.3 "Safe defaults" paragraph |
...
| 11 | ACC-02 blueprint specifies minimum scenario count (≥80) with breakdown per domain | Section 6.1 table |
